...
export type AnswerOutcome =
  ...
  | { status: 'not_found' }
  | { status: 'unavailable' }
  // This person has asked more than the limit allows in the window (2026-09-20). Nothing is
  // persisted and nothing is paid for; the client says to wait a moment.
  | { status: 'rate_limited' }

// How long the same question in the same thread counts as the same question (2026-09-20). A phone
// that lost the connection while the answer was being written posts the question again; within
// this window it gets the answer already given rather than a second one, paid for twice. A minute
// later the same words are a new question, because a person may well ask again.
export const DUPLICATE_QUESTION_WINDOW_MS = 60_000
...
export function createAnswerService(deps: AnswerServiceDeps): AnswerService {
  ...
  const answerOnce = async (
    ...
    // The same question, just answered: the thread's last exchange is this question and its
    // answer, written within the window. The phone that retried gets that answer.
    const [question, answer] = existing.messages.slice(-2)
    if (
      question?.role === 'user' &&
      question.content === content &&
      answer?.role === 'agent' &&
      startedAt.getTime() - answer.createdAt.getTime() <= DUPLICATE_QUESTION_WINDOW_MS
    ) {
      return { status: 'ok', detail: existing }
    }
    if (rateLimiter && !rateLimiter.hit(principal.userId)) {
  ...
  return {
    answer: (principal, threadId, content) => {
      const key = `${principal.userId}\n${threadId}\n${content}`
      const pending = inFlight.get(key)
      if (pending) return pending
      const outcome = answerOnce(principal, threadId, content).finally(() => {
        inFlight.delete(key)
      })

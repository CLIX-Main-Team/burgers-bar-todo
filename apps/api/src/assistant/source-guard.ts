...
// A hostname as it appears in prose. The lookbehind keeps an email address from reading as a
// website and stops a match from starting halfway through a longer name.
const DOMAIN =
  /(?<![@\w.])(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?:il|com|org|net|news|info|io|gov|edu)\b/gi
...
// could not be checked; "according to kolzchut.org.il" says it was read.
const EN_LEAD =
  /(?:\b(?:according to|as per|based on|as reported (?:by|on|in)|reported (?:by|on|in)|cited (?:by|in|on)|published (?:by|on|in)|as stated (?:by|on|in))\s+|\bsources?:\s*)(?:(?:the|an?|its|their)\s+)?(?:(?:official\s+)?(?:web ?)?sites?\s+(?:of\s+)?)?$/i

// lefi, al pi, the abbreviation, and "source:", each optionally wearing a one-letter prefix, then
// optionally "the site (of)". The lookbehind matters: "alfei" (thousands of) contains "lefi".
const HE_LEAD =
  /(?<![\u05d0-\u05ea])(?:[\u05d5\u05e9\u05db]?\u05dc\u05e4\u05d9|[\u05d5\u05e9\u05db]?\u05e2\u05dc[ \u05be-]\u05e4\u05d9|\u05e2\u05e4["\u05f4]\u05d9|\u05de\u05e7\u05d5\u05e8(?:\u05d5\u05ea)?:)\s*(?:\u05d4?\u05d0\u05ea\u05e8(?:\s+\u05d4\u05d0\u05d9\u05e0\u05d8\u05e8\u05e0\u05d8)?\s+(?:\u05e9\u05dc\s+)?)?$/
...
// Hebrew needs the cue in front of it, because the bare letters of "site" also spell the verb "to
// locate", which every honest "I could not find it" uses.
const EN_SITE =
  /\b(?:according to|as per|based on|as reported (?:by|on|in)|reported (?:by|on|in)|cited (?:by|in|on)|published (?:by|on|in))\s+(?:[a-z-]+\s+){0,3}?(?:web ?)?sites?\b/gi
const HE_SITE =
  /(?<![\u05d0-\u05ea])(?:[\u05d5\u05e9\u05db]?\u05dc\u05e4\u05d9|[\u05d5\u05e9\u05db]?\u05e2\u05dc[ \u05be-]\u05e4\u05d9|\u05e2\u05e4["\u05f4]\u05d9)\s+\u05d4?\u05d0\u05ea\u05e8(?:\u05d9\u05dd|\u05d9)?(?![\u05d0-\u05ea])/g
...
// Everything the answer legitimately had in front of it, as one lowercase haystack: the history,
// the question, every tool result, and the sources a tool attached. The system prompt is left
// out on purpose. It names sites to prefer when searching, which is advice, not material, and an
...
const bareHost = (domain: string): string => domain.toLowerCase().replace(/^www\./, '')
...
const scanDomains = (prose: string, material: string): Scan => {
  ...
  let strays = 0
  // Where the last domain that belonged to an attribution ended, so "X and Y" reads as one claim
  // even when X itself turned out to be backed.
  let chainEnd = -1
  let open: UnbackedAttribution | null = null
  for (const match of prose.matchAll(DOMAIN)) {
    const start = match.index
    const end = start + match[0].length
    const backed = material.includes(bareHost(match[0]))
    ...
    const continues = chainEnd >= 0 && JOIN.test(prose.slice(chainEnd, start))
    if (lead === null && !continues) {
      if (!backed) strays += 1
      chainEnd = -1
      open = null
      continue
    ...
    if (open !== null && continues) {
      open.end = end
      open.quote = prose.slice(open.start, end)
      continue
    }
    const from = lead === null ? start : windowStart + lead.index
    open = { kind: 'domain', quote: prose.slice(from, end), start: from, end }
    found.push(open)
  }
  return { found, strays }
}
...
const scan = (draft: SourceGuardDraft): Scan => {
  ...
  const domains = scanDomains(prose, materialOf(draft))
  // "According to the company site" is true once any lookup handed material back, and cannot be
  // told apart from an invention when it did. With nothing looked up at all it can only be one.
  const lookedUp = draft.trace.some((entry) => entry.status === 'ok')
  const sites = lookedUp ? [] : scanSiteWords(prose)
  return {
...
export function repairInstruction(found: UnbackedAttribution[], canSearch: boolean): string {
  ...
  return [
    ...
    `Your draft said ${named}, but no web search ran and no page was opened for this answer, so that attribution is not true.`,
...
// Which language the reader is being answered in, by counting script. The account language is
// the wrong guide: an English account asking in Hebrew is answered in Hebrew.
...
// Remove one attribution from its sentence. Opening a sentence, it takes the comma after it and
// hands its capital to the next word; anywhere else it takes the comma or the space before it.

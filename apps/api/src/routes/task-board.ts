...
export function registerTaskBoardRoutes(app: FastifyInstance, deps: TaskBoardRouteDeps): void {
  ...
  // service then resolves the target location from the principal (a manager or branch admin's own,
  // a super_admin's named one) and enforces the assignee-location invariant before the write
  // (ADR-0007) — never trusting the body's location or assignees blindly. On success the created
  // task (as the creator sees it, assignees hydrated) comes back and the change is announced on the
  // live channel.
  typed.post(
    '/tasks',
    {

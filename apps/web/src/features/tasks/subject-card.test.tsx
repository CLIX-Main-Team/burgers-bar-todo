...
// counts are spoken as words (not only drawn as a rail) and the +N is the API's number rather
// than a second cap; and the menu exists only for a manager of subjects.
...
describe('SubjectCard', () => {
  it('links to the subject, says its counts in words, and prints the overflow the API counted', () => {
    renderCard()
    ...
    expect(screen.getByText('5 open · 4 done')).toBeInTheDocument()
  ...
  it('draws no faces and no overflow for a subject nobody is working in', () => {
    ...
    expect(screen.getByText('0 open · 0 done')).toBeInTheDocument()
